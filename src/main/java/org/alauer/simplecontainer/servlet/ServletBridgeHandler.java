...
package org.alauer.simplecontainer.servlet;

import org.alauer.simplecontainer.servlet.impl.FilterChainImpl;
import org.alauer.simplecontainer.servlet.impl.HttpServletRequestImpl;
import org.alauer.simplecontainer.servlet.impl.HttpServletResponseImpl;
import org.alauer.simplecontainer.servlet.impl.ServletBridgeWebapp;
...
public class ServletBridgeHandler extends IdleStateAwareChannelHandler {
    ...
    public void messageReceived(ChannelHandlerContext ctx, MessageEvent e)
        ...
        final String path = (uri != null ? ServletBridgeWebapp.get().getWebappConfig().getExplodedWarFolder().getAbsolutePath() + File.separator + uri : null);

        if (path != null) {
            final File file = new File(path);
            if (file.exists() && file.isFile())
                handleStaticResourceRequest(file, ctx, e);
            else
                handleHttpServletRequest(ctx, e, request.getUri());
        } else
            handleHttpServletRequest(ctx, e, request.getUri());
    }

    protected void handleHttpServletRequest(ChannelHandlerContext ctx, MessageEvent e, String uri) throws Exception {
        FilterChainImpl chain = ServletBridgeWebapp.get().initializeChain(uri);

        if (!chain.isValid())
            throw new ServletBridgeRuntimeException("Unable to serve " + uri);
    ...
    protected void handleStaticResourceRequest(File file, ChannelHandlerContext ctx,
